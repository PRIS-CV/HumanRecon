...
def run_evaluate():
    ...
    pths = [9]
    [pths.append(pth-1) for pth in range(0,150,20) if pth >0]
    for i in range(len(pths)):
        model_path = os.path.join(model_dir, '{}.pth'.format(pths[i]))
...
def run_visualize():
    ...
    import os
    model_dir = cfg.trained_model_dir
    model_path = os.path.join(model_dir, '{}.pth'.format(149))
    print('load model: {}'.format(model_path))
